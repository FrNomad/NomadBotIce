...
async def on_message(message):
    ...
    if args[0] == hochul:
        ...
        if msg == "외워":
            line = args[2]
            if line == "2호선":
                await message.channel.send(f"**2호선을 외워볼까~요?**")
                if(stop == 1):
                    stop = 0
                ...
                await message.channel.send(f"**3호선을 외워볼까~요?**")
                if(stop == 1):
                    stop = 0
                ...
                await message.channel.send(f"**7호선을 외워볼까~요?**")
                if(stop == 1):
                    stop = 0
                for i in range(0, len(l7)):
                    if(stop == 1):
                        ...
                        break
                    time.sleep(subdly)
                    await message.channel.send(f"**[{l7n[i]}]** {l7[i]}")
                await message.channel.send("**여.기.까.지!**")
            elif len(args) < 3:
